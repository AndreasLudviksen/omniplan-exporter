...
def generate_report(epos, db_name='../omniplan.db'):
    """
    ...
    try:
        with sqlite3.connect(db_name) as conn:
            cursor = conn.cursor()
            parent_task = get_parent_task(epos, db_name)
            if parent_task:
                parent_uid, parent_name, parent_note, start_date, finish_date = parent_task

                create_report_directory()
                report_filename = os.path.join('generated-reports', f"{epos.upper()}.txt")
