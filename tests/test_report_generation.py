...
class TestReportGeneration(unittest.TestCase):
    ...
    def test_report_generation(self):
        ...
        subprocess.run(['python', 'reports/report_jira_task_description.py', 'mup-1', self.output_dir], check=True)
        subprocess.run(['python', 'reports/report_task_assignments_and_status.py', 'mup-24', self.output_dir], check=True)
        subprocess.run(['python', 'reports/report_milestones_top_level.py', self.output_dir], check=True)

        # Verify that the files are created
        for file in self.files_to_check:
            self.assertTrue(os.path.exists(file), f"Expected file {file} was not created.")
